    ...
    <div class="row">
        <div class="card " style="width: 100%">
            <div class="card-body">
                <table class="table">
                    <tr>
                        <div  class="h2 text-success">Продажа <%=add.getCar().getModel()%>,
                    ...
                    <tr>
                        <td>
                            ...
                            <img src="download?name=<%=add.getPhotos().get(0).getName()%>" width="600px" height="400px"/>
                            <% if (add.isStatus()) {%>
                            <div class="font-weight-bold text-danger mt-3">ПРОДАНО</div>
                            <% } else {%>
                            <div class="font-weight-bold text-success mt-3">ПРОДАЕТСЯ</div>
                            <% }%>
                            <div class="mt-3">Дата размещения : <%=add.dateFormat(add.getDate())%></div>
                            <div class="mt-3">Контакты : <%=add.getUser().getName() %>  тел. <%=add.getUser().getPhone() %></div>
                        </td>
                        <td class="border">
                            <div>Модель : <%=add.getCar().getModel()%></div>
                            <div>Двигатель : <%=add.getCar().getEngineType()%>, <%=add.getCar().getEngineVolume()%> л., <%=add.getCar().getEnginePower()%> л.с. </div>
                            <div>Пробег <%=add.getCar().getMileage()%> км.</div>
                            <div>Коробка передач : <%=add.getCar().getTransmission()%></div>
                            <div>Привод <%=add.getCar().getGear()%></div>
                            <div>Дополнительно: <%=add.getDescription()%></div>
                            <div class="h4 text-danger">Цена <%=add.getPrice()%> р</div>
                        </td>
                    </tr>
                    </tbody>
                </table>
